...
func area(height height: Int, width: Int) -> Int {
    let area = height * width
    return area
}

area(height: 10, width: 3)


func getPerimeter(height height: Int, width: Int) -> Int {
    return height * 2 + width * 2
}

getPerimeter(height: 3, width: 5)

func square(height height: Int, width: Int) -> (perimeter: Int, area: Int) {
    let area = height * width
...
let squareA = square(height: 10, width: 5)

print(squareA.area)
print(squareA.perimeter)

func getPerimeterIrregular(leghts lenghts: [Int]) -> Int {
    var totalLenght: Int = 0
    for x in lenghts {
        totalLenght = totalLenght + x
    }
    return totalLenght
}
var arrrayNum = [1,7,10,1,1,21]
print(getPerimeterIrregular(leghts: arrrayNum))

var instructors = ["Joe", "Swayam", "Anusah", "Jorge"]

func getInstruct(prefix prefix: String) -> [String]? {
    var foundIns = [String]()
    for x in instructors {
        if x.hasPrefix(prefix) {
            foundIns.append(x)
        }
    }
    return foundIns
